                    ...
                    <div class="order_detail-top">
                        <div class="row">
                            ...
                            <div class="order_addre-edit col-lg-5 col-md-12">
                                ...
                                <div class="order_detail-proof mt-4">
                                    <div class="proof-of-delivery">
                                        <div class="card">
                                            <div class="card-header bg-white box-header">
                                                <h3>{{ trans('lang.proof_of_delivery') }}</h3>
                                            </div>
                                            <div class="card-body">
                                                <div class="proof-inner">
                                                    <div id="proof_of_delivery">
                                                        <div class="text-center text-muted">{{ trans('lang.no_proof_of_delivery') }}</div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
...
@section('scripts')
    ...
    <script>
        ...
        database.collection('proof_of_delivery').where('orderID', '==', id).get().then(async function(snapshots) {
            if (!snapshots.empty) {
                proofOfDelivery = snapshots.docs[0].data();
                displayProofOfDelivery();
            }
        });
                            ...
                            if (order.hasOwnProperty('scheduleTime') && order.scheduleTime != null) {
                                ...
                                if (now >= windowStart && now <= windowEnd) {
                                    if (isSelfDeliveryGlobally && isSelfDeliveryByVendor && !orderTakeAwayOption) {
                                        // Check if automatic driver assignment is enabled
                                        var autoAssign = true; // This should come from settings
                                        if (autoAssign) {
                                            assignNearestDriver();
                                        } else {
        ...
        async function assignNearestDriver() {
            try {
                jQuery("#data-table_processing").show();
                
                // Get order and vendor location
                const orderSnapshot = await database.collection('restaurant_orders').doc(id).get();
                const orderData = orderSnapshot.data();
                
                // Get vendor location
                const vendorSnapshot = await database.collection('users').where('vendorID', '==', orderData.vendorID).get();
                let restaurantLocation = null;
                if (!vendorSnapshot.empty) {
                    const vendorData = vendorSnapshot.docs[0].data();
                    if (vendorData.location && vendorData.location.latitude && vendorData.location.longitude) {
                        restaurantLocation = vendorData.location;
                    }
                }

                // If no vendor location, use customer address coordinates if available
                let targetLocation = restaurantLocation;
                ...
                if (!targetLocation) {
                    // No location available, fall back to manual assignment
                    $('#assignDriverModal').modal('show');
                    return;
                }

                // Get available drivers
                const driversSnapshot = await database.collection('users')
                    .where('role', '==', 'driver')
                    .where('isActive', '==', true)
                    .where('isDocumentVerify', '==', true)
                    .get();

                if (driversSnapshot.empty) {
                    alert('{{ trans('lang.no_available_drivers') }}');
                    $('#assignDriverModal').modal('show');
                    return;
                }
                ...
                for (let driverDoc of driversSnapshot.docs) {
                    const driverData = driverDoc.data();
                    if (driverData.location && driverData.location.latitude && driverData.location.longitude) {
                        // Check if driver is not already on a delivery
                        const hasActiveOrder = await database.collection('restaurant_orders')
                            .where('driverID', '==', driverDoc.id)
                            .where('status', 'in', ['In Transit', 'Driver Accepted'])
                            .get();

                        if (hasActiveOrder.empty) {
                            ...
                            if (distance < shortestDistance) {
                                ...
                                nearestDriver = { id: driverDoc.id, data: driverData };
                            }
                        }
                    }
                }
                ...
                if (nearestDriver) {
                    ...
                    $('#deliveryman_list').val(nearestDriver.id);
                    $('#assignDriverModal').hide();
                    $('#addPreparationTimeModal').modal('show');
                } else {
                    // No available drivers, show manual assignment
                    alert('{{ trans('lang.no_available_drivers') }}');
                    $('#assignDriverModal').modal('show');
                }
            } catch (error) {
                console.error('Error assigning nearest driver:', error);
        ...
        function displayProofOfDelivery() {
            if (!proofOfDelivery) return;

            var proofHtml = '';

            if (proofOfDelivery.photoUrl) {
                proofHtml += `
                    <div class="mb-3">
                        <h5>{{ trans('lang.delivery_photo') }}</h5>
                        <img src="${proofOfDelivery.photoUrl}" class="img-fluid rounded" style="max-height: 300px;">
                    </div>
                `;
            }

            if (proofOfDelivery.signatureUrl) {
                proofHtml += `
                    <div class="mb-3">
                        <h5>{{ trans('lang.delivery_signature') }}</h5>
                        <img src="${proofOfDelivery.signatureUrl}" class="img-fluid rounded" style="max-height: 200px;">
                    </div>
                `;
            }

            if (proofOfDelivery.deliveryNote) {
                proofHtml += `
                    <div class="mb-3">
                        <h5>{{ trans('lang.delivery_note') }}</h5>
                        <p class="text-muted">${proofOfDelivery.deliveryNote}</p>
                    </div>
                `;
            }

            if (proofOfDelivery.deliveredAt) {
                var deliveredDate = proofOfDelivery.deliveredAt.toDate().toLocaleString();
                proofHtml += `
                    <div class="mb-3">
                        <h5>{{ trans('lang.delivered_at') }}</h5>
                        <p class="text-muted">${deliveredDate}</p>
                    </div>
                `;
            }

            $('#proof_of_delivery').html(proofHtml);
        }
