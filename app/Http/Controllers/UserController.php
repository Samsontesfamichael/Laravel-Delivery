...
class UserController extends Controller
{
    ...
    public function storeAdminSupportUsers(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'name' => 'required|max:255',
            'email' => 'required|email',
        ...
        User::create([
            ...
            'password' => Hash::make($request->input('password')),
            'role_id' => $request->input('role'),
            'uid' => $request->input('uid'), // store firebase doc id
        ]);
    ...
    public function updateAdminUsers(Request $request, $id)
    {
        ...
        $user->role_id = ($id == 1) ? 1 : $request->input('role');

        if ($request->filled('password')) {
            if (!password_verify($request->input('old_password'), $user->password)) {
                return response()->json(['success' => false, 'message' => 'Please enter correct old password'], 422);
            }
            $user->password = Hash::make($request->input('password'));
        }

        // update or clear uid if passed
        if ($request->has('uid')) {
            $user->uid = $request->input('uid') ?: null;
        }
    ...
    {
        return view('settings.users.chat', compact('id'));
    }
}
